...
var topY = 0,
	...
	leftX = 0; //自定义定位偏移量 2007-02-11 由 寒羽枫添加 //选择日期 → 
...
Calendar.prototype.ReturnDate = function(dt) {
	...
	ev = ev.substring(
		...
		((ev.indexOf("ValidatorOnChange();") > 0) ? ev.indexOf("ValidatorOnChange();") + 20 : ev.indexOf("{") + 1), ev.lastIndexOf("}")); //去除验证函数 ValidatorOnChange(); 
...
Calendar.prototype.draw = function() {
	...
	mvAry[mvAry.length] = '    <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:12px;height:40px">';

	mvAry[mvAry.length] = '      <tr>';

	mvAry[mvAry.length] = '        <th align="left" width="5%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:30px;';
	...
	mvAry[mvAry.length] = '        <th align="left" width="40%" style="margin-left:20px"  nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:12px;margin-top:10px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;';
	...
	mvAry[mvAry.length] = '        <th align="right" width="40%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:20px;height:30px;';
	...
	mvAry[mvAry.length] = '  <span >    <select name="calendarHour" id="calendarHour" style="font-size:12px;width:55px;"></select>'+'</span>' + '<span width:10px;>'+Calendar.language["hour"][this.lang]+'</span>';
	...
	mvAry[mvAry.length] = '"><select name="calendarMinute" id="calendarMinute" style="font-size:12px;width:55px;"></select>' +'</span>'+'<span width:10px;>'+ Calendar.language["minute"][this.lang] + '</span>';
	...
	mvAry[mvAry.length] = '"><select name="calendarSecond" id="calendarSecond" style="font-size:12px;width:55px;text-align: center;"></select>' +'</span>'+'<span width:10px;>'+ Calendar.language["second"][this.lang] + '</span>';
	...
	mvAry[mvAry.length] = '        <input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:30px;font-size:12px;cursor:pointer;"/>';
	...
	mvAry[mvAry.length] = '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:30px;font-size:12px;cursor:pointer"/>';

	mvAry[mvAry.length] = '        <input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:30px;font-size:12px;cursor:pointer"/>';
	...
	obj.onclick = function() {
		...
		var today = (calendar.DateMode != pickMode["day"]) ?
			...
			new Date(calendar.year, calendar.month, calendar.day, calendar.hour, calendar.minute, calendar.second) : new Date(); //2008-01-29 
...
Calendar.prototype.show = function(dateObj, popControl) {
	...
	if (this.panel.innerHTML == "" || cal.dateFormatStyleOld != cal.dateFormatStyle) //，则重新初始化 
	...
	//修改 → 把 visibility 变为 display，并添加失去焦点的事件 //this.setDisplayStyle("select", "hidden"); 
...

{
	...
	var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:260px;font-size:12px;"></div>';
