const Province = [
    { name: '北京', pingyin: 'BeiJing', city: [{ name: '北京', area: ['东城区', '西城区', '崇文区', '宣武区', '朝阳区', '丰台区', '石景山区', '海淀区', '门头沟区', '房山区', '通州区', '顺义区', '昌平区', '大兴区', '平谷区', '怀柔区', '密云县', '延庆县'] }] },

    { name: '天津', pingyin: 'TianJin', city: [{ name: '天津', area: ['和平区', '河东区', '河西区', '南开区', '河北区', '红桥区', '塘沽区', '汉沽区', '大港区', '东丽区', '西青区', '津南区', '北辰区', '武清区', '宝坻区', '宁河县', '静海县', '蓟  县'] }] },
...
function yeluochenCity(id, callBack, city) {
    ...
    const boxId = `${id}-${new Date().getTime()}`;
    box.setAttribute('id', boxId);
    // 设置css
    $('body').append(box);
    // 固定省份文本
    function createProvincesText(provinces) {
        const pingyin1 = ['A', 'B', 'C', 'D', 'E', 'F', 'G'];
        const pingyin2 = ['H', 'I', 'J', 'K'];
        const pingyin3 = ['L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S'];
        const pingyin4 = ['T', 'U', 'V', 'W', 'X', 'Y', 'Z'];
        let AGtext = '';
        let HKtext = '';
        let LStext = '';
        let TZtext = '';
        for (let i = 0; i < provinces.length; i += 1) {
            const provinceUp = provinces[i].pingyin.split('')[0].toLocaleUpperCase();
            if (pingyin1.indexOf(provinceUp) !== -1) {
                AGtext += `<span data-index=${i}>${provinces[i].name}</span>`;
            } else if (pingyin2.indexOf(provinceUp) !== -1) {
                HKtext += `<span data-index=${i}>${provinces[i].name}</span>`;
            } else if (pingyin3.indexOf(provinceUp) !== -1) {
                LStext += `<span data-index=${i}>${provinces[i].name}</span>`;
            } else if (pingyin4.indexOf(provinceUp) !== -1) {
                TZtext += `<span data-index=${i}>${provinces[i].name}</span>`;
            }
        }
        return {
            AG: AGtext,
    ...
    // 导航+大概框架+基础样式
    const navText = `<ul class="city-nav">
                        <li class="J-city-nav" data-id="${id}-city-provinces">省份</li>
                        <li class="J-city-nav" data-id="${id}-city-citys">城市</li>
                        <li class="J-city-nav" data-id="${id}-city-districts">县区</li>
                    </ul>
                    <div class="city-items">
                        <div class="items-box J-items-box" id="${id}-city-provinces">
                        </div>
                        <div class="items-box J-items-box" id="${id}-city-citys">
                        </div>
                        <div class="items-box J-items-box" id="${id}-city-districts">
                        </div>
                    </div>
                    `;
    $(`#${boxId}`).html(navText);
    ...
    $(`#${boxId} .city-nav li:nth-child(3)`).css({
        borderRight: 'none'
    });
    $(`#${boxId} .city-nav li:nth-child(1)`).css({
        borderBottom: 'none',
        backgroundColor: '#fff'
    });
    $(`#${boxId} .city-items`).css({
        margin: 0,
        padding: '10px 15px',
        overFlow: 'hidden'
    });
    ...
    function setClass() {
        $(`#${boxId} .items-box dl`).css({
            overflow: 'hidden'
        });
        $(`#${boxId} .items-box dt`).css({
            margin: '0',
            paddingTop: '2px',
            float: 'left',
            width: '15%'
        });
        $(`#${boxId} .items-box dd`).css({
            margin: '0',
            paddingTop: '0',
            float: 'left',
            width: '85%'
        });
        $(`#${boxId} .items-box span`).css({
            float: 'left',
            padding: '2px 10px',
            whiteSpace: 'nowrap',
            borderRadius: '4px'
    ...
    function createBoxText() {
        callBack(userCity);
        let str = '';
        for (const key in userCity) {
            if (userCity[key]) {
                str += `${userCity[key]}${key !== 'district' ? '<span class="J-result-border">/</span>' : ''}`;
            }
        }
        target.html(str);
    ...
    // 被选择的样式
    function setSpanClass(father, span) {
        father.find('span').css({
    ...
    $(`#${id}-city-provinces`).show();
    function changeNav(num) {
        $(`#${boxId} .J-city-nav`).css({
            borderBottom: '1px solid #ccc',
            backgroundColor: 'transparent'
        });
        $(`#${boxId} .city-nav .J-city-nav:nth-child(${num})`).css({
            borderBottom: 'none',
            backgroundColor: '#fff'
        });
    }
    function createProvinces() {
        const provinces = createProvincesText(Province);
        const str = `
                    <dl class="">
                        <dt>A-G</dt>
                        <dd class="">${provinces.AG}</dd>
                    </dl>
                    <dl class="">
                        <dt>H-K</dt>
                        <dd class="">${provinces.HK}</dd>
                    </dl>
                    <dl class="">
                        <dt>L-S</dt>
                        <dd class="">${provinces.LS}</dd>
                    </dl>
                    <dl class="">
                        <dt>T-Z</dt>
                        <dd class="">${provinces.TZ}</dd>
                    </dl>
                    `;
        $(`#${id}-city-provinces`).html(str);
        setClass();
        // 清理掉市/县/区
        $(`#${boxId} #${id}-city-citys`).html('');
        $(`#${boxId} #${id}-city-districts`).html('');
    }
    createProvinces();
    // 被选中的城市(市)
    let selectCitys;
    ...
    function createDistricts(districts) {
        ...
        districts.forEach((value) => {
            str += `<span>${spaceTrim(value, true)}</span>`;
        });
        // console.log(str);
        $(`#${id}-city-districts`).html(str);
        $(`#${boxId} .J-items-box`).hide();
        $(`#${id}-city-districts`).show();
        setClass();
        changeNav(3);
    }
    // 创建市
    function createCitys(province, citys) {
        // console.log(citys);
        let str = '';
        citys.forEach((value, index) => {
            str += `<span data-index="${index}">${spaceTrim(value.name, true)}</span>`;
        });
        $(`#${id}-city-citys`).html(str);
        // 清理掉县/区
        $(`#${boxId} #${id}-city-districts`).html('');
        if (['北京', '上海', '重庆', '天津'].indexOf(province) === -1) {
            $(`#${boxId} .J-items-box`).hide();
            $(`#${id}-city-citys`).show();
            setClass();
            changeNav(2);
        } else {
            $(`#${id}-city-citys span`).css({
                backgroundColor: '#f60',
                color: '#fff'
            });
            createDistricts(selectCitys[0].area);
        }
    }
    let first = true;
    function addEvent() {
        target.on('click', function () {
            if (first) {
                const elClientRect = document.getElementById(id).getBoundingClientRect();
                $(`#${boxId}`).css({
                    margin: 0,
                    padding: 0,
                    ...
                    position: 'absolute',
                    boxSizing: 'border-box',
                    top: `${target.offset().top + (elClientRect.height - 1)}px`,
        ...
        $(`#${boxId}`).on('click', '.J-city-nav', function () {
            $(`#${boxId} .J-city-nav`).css({
                borderBottom: '1px solid #ccc',
                backgroundColor: 'transparent'
            });
            ...
            $(`#${boxId} .items-box`).hide();
            $(`#${$(this).data('id')}`).show();
        });
        $(`#${boxId} #${id}-city-provinces`).on('click', 'span', function () {
            setSpanClass($(`#${id}-city-provinces`), $(this));
            const province = $(this).html();
            selectCitys = Province[$(this).data('index')].city;
            createCitys(province, selectCitys);
            // 用户选择
            userCity.province = province;
            userCity.city = null;
            userCity.district = null;
            createBoxText();
        });
        $(`#${boxId} #${id}-city-citys`).on('click', 'span', function () {
            setSpanClass($(`#${id}-city-citys`), $(this));
            selectDistricts = selectCitys[$(this).data('index')].area;
            createDistricts(selectDistricts);
            // 用户选择
            userCity.city = $(this).html();
            userCity.district = null;
            createBoxText();
        });
        $(`#${boxId} #${id}-city-districts`).on('click', 'span', function () {
            setSpanClass($(`#${id}-city-districts`), $(this));
            // 用户选择
            userCity.district = $(this).html();
            createBoxText();
            $(`#${boxId}`).hide();
    ...
    return {
        ...
        destroy() {
            $(`#${boxId}`).off();
            $(`#${boxId} #${id}-city-provinces`).off();
            $(`#${boxId} #${id}-city-citys`).off();
            $(`#${boxId} #${id}-city-districts`).off();
            $(`#${boxId}`).remove();
        }
    };
}